   ...
   */
	async execute(interaction) {
		try {
			...
			if (memberToMute.permissions.has(PermissionFlagsBits.ManageRoles)) {
				return await interaction.reply({
			...
			// Verificar si la respuesta inicial fue enviada con éxito
			if (initialResponse) {
				// Eliminar la respuesta después de un tiempo
				setTimeout(async () => {
					try {
						await interaction.deleteReply();
					}
					catch (deleteError) {
						console.error('Error al eliminar la respuesta:', deleteError);
					}
				}, 3000);
			}
