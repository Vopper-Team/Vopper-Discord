...
const { messageInfo, messageError, messageSuccess, messageWarning, messageCoupon } = require(path.join(
  process.cwd(),
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('código').setDescription('Selecciona la plataforma que corresponda al cupón').setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('expiración')
        .setDescription('Fecha de expiración del cupón. Formato: yyyy-mm-dd-hh')
        .setRequired(true)
    )
    .addStringOption(option =>
      option.setName('url').setDescription('Coloque la url para acceder al curso.').setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('descripción')
  ...
  async execute (interaction) {
    try {
      ...
      const missingArguments = ['plataforma', 'código', 'expiración', 'url', 'descripción'].filter(
      ...
      if (channelCoupon) {
        await channelCoupon.send({
          embeds: [
            messageCoupon('¡Cupón disponible, aprovechalo!', textDescription, {
              text: `Gracias a ${interaction.member.displayName}`,
